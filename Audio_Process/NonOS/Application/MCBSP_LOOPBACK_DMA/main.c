...
/*
 * 功能描述：
 *     本例程通过DMA实现MCBSP内部回环字符数据，利用DMA将数据从缓冲区sdata到MCBSP发送，
 *  然后从MCBSP接收到缓冲区rdata。用户可通过修改宏定义WORD_SIZE选择测试8位、16位、32位
 *  模式回环。
...
// This function is found in DSP2833x_PieVect.c.
   ...
   SCIStdioInit();
   SCIPuts("\r\n ============Test Start===========.\r\n", -1);
   SCIPuts("Welcome to TL28335 MCBSP_LOOPBACK_DMA Demo application.\r\n\r\n", -1);
