 ...
 * 功能描述：
 *     ADC序列覆盖模式采样。
 *     本例程通过A0通道进行持续采样，将数据结果储存至数组变量SampleTable中，
 *     并在每个序列标识反转GPIO8引脚电平。
 *     排序器工作在覆盖模式，可通过变量SampleTable获取转换结果。
 *
 */
...
// Enable the pin GPIO8 as output
   EALLOW;
   GpioCtrlRegs.GPAMUX1.bit.GPIO8 = 0;    // GPIO pin
   GpioCtrlRegs.GPADIR.bit.GPIO8 = 1;     // Output pin
   EDIS;
...
// This function is found in DSP2833x_PieVect.c.
   ...
   SCIStdioInit();
   SCIPuts("\r\n ============Test Start===========.\r\n", -1);
   SCIPuts("Welcome to TL28335 ADC_SEQ_OVD_TEST Demo application.\r\n\r\n", -1);
...
// Start SEQ1
   ...
   {  // Take ADC data and log them in SampleTable array
     ...
     for (i=0; i<(BUF_SIZE/16); i++)
     {
       ...
       GpioDataRegs.GPASET.bit.GPIO8 = 1;  // Set GPIO8 for monitoring  -optional
       ...
       while (AdcRegs.ADCST.bit.INT_SEQ1== 0){}
 	   GpioDataRegs.GPACLEAR.bit.GPIO8 = 1;  // Clear GPIO8 for monitoring  -optional
       AdcRegs.ADCST.bit.INT_SEQ1_CLR = 1;
